 ...
 *
 * This is the ROOT-LEVEL entry point used when the SDK is installed via GitHub:
 *   npm install github:neerajj911/flutter-chat-sdk#vX.Y.Z
 *
 * It patches the host RN app's Android files automatically.
 *
 * What it does:
 *   1. Fixes sdk.dir backslashes in local.properties
 *   2. Points gradle-wrapper to a minimum Gradle version
 ...
 *   7. Ensures android/gradle.properties has useAndroidX, JVM heap flags
 */

const fs   = require('fs');
const path = require('path');

// When installed via GitHub, __dirname = node_modules/react-native-flutter-chat/
// So ROOT = node_modules/react-native-flutter-chat/../../  =  host app root
const PKG_DIR  = path.resolve(__dirname);
const ROOT     = path.resolve(PKG_DIR, '..', '..');
const ANDROID  = path.join(ROOT, 'android');

// The android + flutter_repo dirs are nested inside the repo subfolder
const SDK_ANDROID    = path.join(PKG_DIR, 'flutter-chat-sdk', 'react-native-flutter-chat', 'android');
const SDK_FLUTTER_REPO = path.join(SDK_ANDROID, 'libs', 'flutter_repo');
...
function fixSettingsGradle() {
  ...
  // Path to android dir is nested when installed via GitHub
  const block = `
include ':react-native-flutter-chat'
project(':react-native-flutter-chat').projectDir = new File(
    rootProject.projectDir,
    '../node_modules/react-native-flutter-chat/flutter-chat-sdk/react-native-flutter-chat/android'
)
`;
...
function fixRootBuildGradle() {
  ...
  // Path to flutter_repo is nested when installed via GitHub
  const block = `
allprojects {
    repositories {
        google()
        mavenCentral()
        maven {
            url uri("$rootDir/../node_modules/react-native-flutter-chat/flutter-chat-sdk/react-native-flutter-chat/android/libs/flutter_repo")
        }
        maven {
...
function fixGradleProperties() {
  ...
  write(file, content);
  ok('android/gradle.properties — useAndroidX set, enableJetifier removed, JVM heap set to 4096m');
}
